...
export class ExcelService {
  ...
  async generateFrameWorkExcel(data, header, title) {  
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet(this.localizationService.instant('::Report') , {
      pageSetup:{
        horizontalCentered:true,
        verticalCentered:true
      }
    ...
    // Add Row and formatting
    const titleRow = worksheet.addRow([this.localizationService.instant('::Report')  + ' ' + title ]);
    titleRow.font = { name: 'Arial', family: 4, size: 16, underline: 'double', bold: true };
    worksheet.addRow([]);
    worksheet.addRow([]);
    // const subTitleRow = worksheet.addRow(['Date : ' + this.datePipe.transform(new Date(), 'medium')]);
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      const blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      fs.saveAs(blob, this.localizationService.instant('::Report')  + ' - ' + title + '.xlsx');
    });
